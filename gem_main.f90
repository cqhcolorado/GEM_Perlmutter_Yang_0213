...
!  grid1_tot_tm=grid1_end_tm0-grid1_start_tm0+grid1_end_tm1-grid1_start_tm1
  grid1_tot_tm=grid1_end_tm0-grid1_start_tm0
  ...
  load_tm=load_tm/numprocs
   ...
   if(myid==0)write(*,*),'total_tm',total_tm,'poisson_tm',poisson_tm,'ppush_tm',ppush_tm,'cpush_tm',cpush_tm,'pmove_tm',pmove_tm,'grid1_tm',grid1_tm,'others',total_tm-poisson_tm-ppush_tm-cpush_tm-pmove_tm
...
!$omp target update to(gxdgy,dydr,dbdr,dbdth,dqhdr,jacob)
!$omp target update to(yfn,hght,thflx)
!$omp target update to(rmaj,rmajp,elon,selon,tria,stria, psi)
...
!$omp target update to(e1gx,e1gy,e2gx,e2gy,bdge1gx,bdge1gy,bdge2gx,bdge2gy)
!$omp target update to(psip2)
!$omp target update to(curvbz,srbr,srbz,thbr,thbz,prsrbr,prsrbz,pthsrbr,pthsrbz,bdcrvb)
...
#ifdef OPENMP
!$omp target data map(to:jfn(:),vparsp(ns,:),f(:),dipdr(:),aparh(:,:,:),fisrc(ns,:),g0i(:,ns),y2(:,ns),pzi(:,ns),lr(1),u0i(:,ns),delbyh(:,:,:),xii(:,ns),capns(ns,:),phincp(:),gxdgy(:,:),grcgt(:,:),capts(ns,:),gmrkr(ns,:,:),dtiz(ns,:),bdcrvb(:,:),dbdr(:,:),tgis(ns),z0i(:,ns),dahdz(:,:,:),gr(:,:),qhat(:,:),q(ns),grdgt(:,:),avwixez(ns,:,:),ez(:,:,:),psi(:),sf(:),bfld(:,:),nonlin(ns),psip(:),mm(ns),curvbz(:,:),delbxh(:,:,:),ey(:,:,:),eki(:,ns),xn0s(ns,:),ex(:,:,:),dbdth(:,:),dnisrcz(ns,:),apar(:,:,:),delby(:,:,:),delbx(:,:,:),dadz(:,:,:),t0s(ns,:),avwixeps(ns,:,:),dnisrc(ns,:),dpdz(:,:,:),thfnz(:),mu(:,ns),psip2(:),pzcrit(ns),dydr(:,:),radius(:,:),mims(ns)) map(tofrom:w2(:,ns),u2(:,ns),u3(:,ns),w3(:,ns),mynopi,x2(:,ns),z3(:,ns),z2(:,ns),x3(:,ns),y3(:,ns))
...
#ifdef OPENMP
!$omp target data map(to:jfn(:),vparsp(ns,:),dpdz(:,:,:),dnisrc(ns,:),avwixeps(ns,:,:),t0s(ns,:),dadz(:,:,:),delbx(:,:,:),delby(:,:,:),apar(:,:,:),dnisrcz(ns,:),dbdth(:,:),ex(:,:,:),dydr(:,:),ey(:,:,:),delbxh(:,:,:),curvbz(:,:),mm(ns),nonlin(ns),delbyh(:,:,:),bfld(:,:),sf(:),psi(:),ez(:,:,:),avwixez(ns,:,:),grdgt(:,:),q(ns),qhat(:,:),gr(:,:),dahdz(:,:,:),tgis(ns),dbdr(:,:),bdcrvb(:,:),dtiz(ns,:),gmrkr(ns,:,:),capts(ns,:),grcgt(:,:),gxdgy(:,:),phincp(:),capns(ns,:),psip(:),lr(1),psip2(:),xn0s(ns,:),thfnz(:),g0i(:,ns),fisrc(ns,:),aparh(:,:,:),dipdr(:),f(:),radius(:,:),mu(:,ns),mims(ns)) map(tofrom:mynos,y3(:,ns),w3(:,ns),mypfl_es(:),y2(:,ns),z3(:,ns),z2(:,ns),x2(:,ns),x3(:,ns),u2(:,ns),u3(:,ns),w2(:,ns),mypfl_em(:),mynowi,myefl_em(:),myavewi,myefl_es(:),myke)
...
#ifdef OPENMP
!$omp target data map(tofrom:mydt(:,:,:),myden(:,:,:),myjpar(:,:,:)) map(to:gxdgy(:,:),thfnz(:),grcgt(:,:),q(ns),qhat(:,:),gr(:,:),z3(:,ns),y3(:,ns),w3(:,ns),radius(:,:),f(:),bfld(:,:),lr(1),mm(ns),mu(:,ns),u3(:,ns),x3(:,ns),mims(ns))
!$omp target teams loop private(rhox,rhoy) thread_limit(128)
#endif
...
#ifdef OPENMP
!$omp target data map(tofrom:mydt(:,:,:),mydene(:,:,:),myupar(:,:,:)) map(to:u3e(1:mme),w011(1:mme),w001(1:mme),w101(1:mme),w000(1:mme),w100(1:mme),w010(1:mme),w110(1:mme),x3e(1:mme),w111(1:mme),thfnz(:),bfld(:,:),apar(:,:,0:1),z3e(1:mme),y3e(1:mme),w3e(1:mme),mue3(1:mme))
...
#ifdef OPENMP  
!$omp target data map(to:radius(:,:),thfnz(:),jfn(:),dydr(:,:),psip2(:),dphidt(:,:,0:1),dnesrc(:),avwexeps(:,:),mue(1:mme),dadz(:,:,0:1),delbx(:,:,0:1),delby(:,:,0:1),apar(:,:,0:1),nue0(:),curvbz(:,:),dpdz(:,:,0:1),dbdth(:,:),ex(:,:,0:1),xn0e(:),u0e(1:mme),zeff(:),delbyh(:,:,0:1),phi(:,:,0:1),bfld(:,:),sf(:),psi(:),ez(:,:,0:1),avwexez(:,:),pze(1:mme),qhat(:,:),gr(:,:),dahdz(:,:,0:1),z0e(1:mme),t0e(:),dbdr(:,:),bdcrvb(:,:),dtez(:),gmrkre(:,:),capte(:),grcgt(:,:),grdgt(:,:),phincp(:),capne(:),xie(1:mme),psip(:),eke(1:mme),ey(:,:,0:1),delbxh(:,:,0:1),dnesrcz(:),y2e(1:mme),g0e(1:mme),fesrc(:),aparh(:,:,0:1),dipdr(:),f(:)) map(tofrom:y3e(1:mme),x3e(1:mme),mue2(1:mme),z2e(1:mme),x2e(1:mme),z3e(1:mme),w3e(1:mme),myopz,u2e(1:mme),u3e(1:mme),w2e(1:mme),myoen,myavptch,mue3(1:mme),myaven)
!$omp target teams loop
#endif
...
#ifdef OPENMP  
!$omp target data map(tofrom:mue3(1:mme),myke,myefl_es(:),myavewe,myefl_em(:),mypfl_es(:),mynowe,mypfl_em(:),w2e(1:mme),mytrap,x3e(1:mme),ipass(1:mme),x2e(1:mme),z2e(1:mme),z3e(1:mme),u2e(1:mme),y2e(1:mme),w3e(1:mme),y3e(1:mme),u3e(1:mme),mynos,myptrp,mytotn) map(to:radius(:,:),f(:),dipdr(:),aparh(:,:,0:1),fesrc(:),g0e(1:mme),thfnz(:),xn0e(:),dnesrcz(:),delbxh(:,:,0:1),ey(:,:,0:1),dydr(:,:),psip(:),capne(:),phincp(:),grcgt(:,:),capte(:),gmrkre(:,:),dtez(:),bdcrvb(:,:),dbdr(:,:),t0e(:),dahdz(:,:,0:1),gr(:,:),qhat(:,:),psip2(:),avwexez(:,:),ez(:,:,0:1),psi(:),sf(:),bfld(:,:),phi(:,:,0:1),delbyh(:,:,0:1),zeff(:),ex(:,:,0:1),dbdth(:,:),dpdz(:,:,0:1),curvbz(:,:),nue0(:),apar(:,:,0:1),delby(:,:,0:1),delbx(:,:,0:1),dadz(:,:,0:1),mue2(1:mme),jfn(:),avwexeps(:,:),dnesrc(:),dphidt(:,:,0:1),grdgt(:,:))
...
subroutine jie(ip,n)
  ...
  do ns = 1,nsm
     ...
     mydnidt = 0.
#ifdef OPENMP
!$omp target data map(tofrom:mydnidt(:,:,:),myjpar(:,:,:),myjpex(:,:,:),myjpey(:,:,:)) map(to:t0s(ns,:),mu(:,ns),vparsp(ns,:),psip2(:),mm(ns),lr(:),delby(:,:,:),u3(:,ns),x3(:,ns),capns(ns,:),phincp(:),ey(:,:,:),bdge1gy(:,:),bdcrvb(:,:),e1gy(:,:),dydr(:,:),apar(:,:,:),grdgt(:,:),q(ns),qhat(:,:),gr(:,:),curvbz(:,:),z3(:,ns),y3(:,ns),jfn(:),w3(:,ns),tgis(ns),dbdr(:,:),bdge2gy(:,:),dipdr(:),gmrkr(ns,:,:),cn(:),grcgt(:,:),e1gx(:,:),e2gx(:,:),bdge1gx(:,:),bdge2gx(:,:),gxdgy(:,:),sn(:),bfld(:,:),nonlin(ns),psip(:),thfnz(:),dbdth(:,:),e2gy(:,:),xn0s(ns,:),ex(:,:,:),delbx(:,:,:),g0i(:,ns),f(:),capts(ns,:),radius(:,:),mims(ns))
!$omp target teams loop private(rhox,rhoy) private(gdum) thread_limit(128)
#endif
...
#ifdef OPENMP  
!$omp target data map(tofrom:myupazd(:,:,:),myupex(:,:,:),mydnedt(:,:,:),myupey(:,:,:)) map(to:w111(1:mme),delbx(:,:,0:1),ey(:,:,0:1),u3e(1:mme),w011(1:mme),w001(1:mme),w101(1:mme),w000(1:mme),w100(1:mme),w010(1:mme),w110(1:mme),ex(:,:,0:1),xn0e(:),jfn(:),delby(:,:,0:1),x3e(1:mme),capne(:),phincp(:),ez(:,:,0:1),apar(:,:,0:1),psip2(:),qhat(:,:),gr(:,:),curvbz(:,:),z3e(1:mme),y3e(1:mme),w3e(1:mme),t0e(:),dbdr(:,:),bdcrvb(:,:),dipdr(:),gmrkre(:,:),capte(:),grcgt(:,:),grdgt(:,:),radius(:,:),bfld(:,:),phi(:,:,0:1),psip(:),thfnz(:),g0e(1:mme),f(:),dydr(:,:),dbdth(:,:),mue3(1:mme))

!$omp target teams loop private(gdum) thread_limit(128)
#endif
...
#ifdef OPENMP
!$omp target data map(tofrom:myden0(:,:,:),myupa(:,:,:),myupa0(:,:,:)) map(to:f(:),g0e(1:mme),dbdr(:,:),grcgt(:,:),gmrkre(:,:),psip(:),bfld(:,:),mue3(1:mme),jfn(:),apar(:,:,0:1),radius(:,:),u3e(1:mme),w011(1:mme),w001(1:mme),w101(1:mme),w000(1:mme),w100(1:mme),w010(1:mme),w110(1:mme),z3e(1:mme),y3e(1:mme),w111(1:mme),xn0e(:),thfnz(:),t0e(:),x3e(1:mme))
!$omp target teams loop thread_limit(128)
#endif
...
#ifdef OPENMP  
!$omp target data map(tofrom:myavw(:,:),myg(:,:)) map(to:mue3(1:mme),u3e(1:mme),x3e(1:mme),z3e(1:mme),w3e(1:mme),t0e(:),bfld(:,:),thfnz(:))
!$omp target teams loop thread_limit(128)
#endif
...
#ifdef OPENMP
  !$omp target data map(tofrom:mynclambda(:,:),myavw(:,:),myg(:,:)) map(to:t0s(ns,:),w3(:,ns),x3(:,ns),z3(:,ns),mm(ns),thfnz(:),jac(:,0),u3(:,ns),mu(:,ns),bfld(:,:),mims(ns))
  !$omp target teams loop thread_limit(128)
#endif
