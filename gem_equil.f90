...
!$acc enter data create( gxdgy,dydr,dbdr,dbdth,dqhdr,jacob)
!$acc enter data create( yfn,hght,thflx) 
!$acc enter data create( rmaj,rmajp,elon,selon,tria,stria, psi)
...
#ifdef OPENMP
!$omp target enter data map(alloc:bfld,qhat,radius,gr,gth,grdgt,grcgt)
!$omp target enter data map(alloc:gxdgy,dydr,dbdr,dbdth,dqhdr,jacob)
!$omp target enter data map(alloc:yfn,hght,thflx)
!$omp target enter data map(alloc:rmaj,rmajp,elon,selon,tria,stria, psi)
!$omp target enter data map(alloc:f,psip,sf,jacoba,jfn,zfnth,thfnz)
!$omp target enter data map(alloc:t0i,t0e,t0b,t0c,t0ip,t0ep,t0bp,t0cp)
!$omp target enter data map(alloc:xn0i,xn0e,xn0c,xn0b,xn0ip,xn0ep,xn0bp)
!$omp target enter data map(alloc:xn0cp,vpari,vparc,vparb)
!$omp target enter data map(alloc:vparip,vparcp,vparbp)
!$omp target enter data map(alloc:capti,capte,captb,captc,capni,capne)
!$omp target enter data map(alloc:capnb,capnc,zeff,nue0,phinc,phincp)
!$omp target enter data map(alloc:er,upari,dipdr)
!$omp target enter data map(alloc:e1gx,e1gy,e2gx,e2gy,bdge1gx,bdge1gy,bdge2gx,bdge2gy)
!$omp target enter data map(alloc:psip2)
!$omp target enter data map(alloc:curvbz,srbr,srbz,thbr,thbz,prsrbr,prsrbz,pthsrbr,pthsrbz,bdcrvb)
!$omp target enter data map(alloc:t0s,xn0s,capts,capns,vpars,vparsp)
!$omp target enter data map(alloc:cn0s,n0smax,tgis)
#endif
      !Normalization
      e = 1.6e-19
      proton = 1.67e-27
